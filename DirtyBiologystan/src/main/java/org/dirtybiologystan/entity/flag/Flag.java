...
public class Flag {
	...
	public void chargerDataFromFouloscopieAndCodati() throws Exception {
		...
		String data = new BufferedReader(new FileReader(DeployInit.PathResourcesDeploy + "/templates/flag/flag.html"))
				.readLine();
		String jsonString = gson.toJson(data);
		List<LinkedTreeMap<?, ?>> lofficiel = gson.fromJson(data, List.class);
		// En date du 5 décembre
		data = new BufferedReader(new FileReader(DeployInit.PathResourcesDeploy + "/templates/flag/codati.html"))
				.readLine();
		...
		/*
		 * for (LinkedTreeMap<?, ?> el : lofficiel) { Double indexInFlag = (Double)
		 * el.get("indexInFlag"); map.put(indexInFlag.intValue(), el);
		 * System.out.println("Rajout à l'index"+ indexInFlag.intValue());
		 * //System.out.println(map.get(1));
		 * 
		 * }
		 * 
		 * for (LinkedTreeMap<?, ?> el : lcodati) { Double indexInFlag = (Double)
		 * el.get("indexInFlag"); Double ligne = (Double) el.get("x"); Double colone =
		 * (Double) el.get("y"); Pixel p = new
		 * Pixel(ligne.intValue(),colone.intValue());
		 * 
		 * if (map.containsKey(indexInFlag.intValue())) { LinkedTreeMap<?, ?> oo =
		 * map.get(indexInFlag.intValue()); String author = (String) oo.get("author");
		 * String hexColor = (String) oo.get("hexColor").toString(); if
		 * (!(author.compareTo((String) el.get("author"))==0)) { authorError.add(p); }
		 * else if (!(hexColor.compareTo((String) el.get("hexColor"))==0)) {
		 * hexColorError.add(p); map.put(indexInFlag.intValue(), el); }else {
		 * map.put(indexInFlag.intValue(), el); }
		 * 
		 * }else { System.out.println("map non found " + indexInFlag); map.put(((Double)
		 * el.get("index")).intValue(), el);//Si on prend les données de codati } }
		 */
		Integer k = 0;
	...
	private void rajouterPixelFromData(Pixel p) {
		...
		pixies.add(p);/*
						 * int x = p.getLigne()-1; if (x > drapeau.size()) { ArrayList<Pixel> ligne =
						 * new ArrayList<Pixel>(); ligne.add(p); drapeau.add(ligne); }else {
						 * drapeau.get(x).add(p); }
						 */
		try {
			int x = p.getLigne();
	...
	public synchronized Pixel rajouterNewPixel(String couleur) throws Exception {
		...
		// Ensuite...
		/**
		 * Si nous somme dans l'état colone on vérifie que l'on atteint pas la limite du
		 * ratio
		 */
	...
	public float calculRatioOverflow() {
		...
		// float yflag = drapeau.get(drapeau.size()).size();//taille en colone de la
		// dernière ligne
	...
	public Pixel getPixel(Integer ligne, Integer colone) {
		return drapeau.get(ligne).get(colone);
	}

	public Pixel getPixel(String ligne, String colone) {
		try {
			return this.getPixel(Integer.parseInt(ligne),Integer.parseInt(colone));
		} catch (Exception e) {
			return null;
		}
